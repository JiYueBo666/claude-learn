...
def _get_intro_section() -> str:
    """角色定义（AI 编程助手）+ 安全红线（禁止恶意用途、禁止猜测 URL）"""
    return (
...
def _get_system_section() -> str:
    """系统规则（工具权限模式、<system-reminder> 标签处理、hook 机制、自动压缩）"""
    items = [
...
def _get_doing_tasks_section() -> str:
    """任务执行规范（不读代码不改代码、不要过度工程、只在边界验证、信任内部代码...）"""
    items = [
...
def _get_actions_section() -> str:
    """风险操作确认原则（可逆操作自由做，不可逆操作要确认，measure twice cut once）"""
...
def _get_using_tools_section() -> str:
    """工具使用偏好（优先用专用工具而非 Bash、Todo 管理、并行调用）"""
    tool_prefs = [
...
def _get_tone_and_style_section() -> str:
    """语气和格式（不用 emoji、简洁、引用格式 file:line、不用冒号引出工具调用）"""
    items = [
...
def _get_env_section(cwd: str, model: str = "") -> str:
    """动态环境信息（工作目录、是否 git、平台、Shell、OS、模型）"""
    is_git = False
    try:
